...
const MarketingRecord = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('http://localhost:5000/api/markrecord');
    ...
    const handleSaveEditedData = async (index) => {
        try {
            ...
            await axios.post('http://localhost:5000/api/update-markdata', editedData);
   ...
   const getStatusColor = (status) => {
    ...
    return (
        <div>
            <h2>MARKETING FORM DATA LIST</h2>
             <StatusLegend/>
            <table>
                <thead>
                    <tr>
